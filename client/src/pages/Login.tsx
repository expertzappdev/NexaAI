  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success && response.data.token) {
        login(response.data.token, response.data.email || email, response.data.name || 'User');
        navigate('/');
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-zinc-950 px-4 py-12 transition-colors">
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1.5">Sign in to resume your conversations</p>
        </div>
